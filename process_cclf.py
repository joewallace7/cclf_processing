...
def import_records(thefile,theformat):
    ...
    cnx.execute(f"create table {theformat} ({column_list_str});")
    cnx.commit()

    # Import the records using the column specifications
    counter = 0
    data = []
    with open(thefile,'r') as infile: # Open the file
        for line in infile: # Cycle through the file lines
            line = line.strip('\n')
            linedata = {}
            linedata_list = []
            for c in format_info:
                linedata[c[0]] = str(line[c[1]-1:c[2]]).strip()
            #print(linedata)
            cnx.execute(f"insert into {theformat} values ({('?,'*len(format_info)).strip(',')})",list(linedata.values()))
            #breakpoint()
            counter+=1
            if counter%2000 == 0:
                print(theformat,':',counter)
                cnx.commit()
            pass
    try:
        cnx.commit()
        ...
        pass
    print(theformat,'total rows imported:',counter)
    return
